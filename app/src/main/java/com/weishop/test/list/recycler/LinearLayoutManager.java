...
import com.weishop.test.list.recycler.RecyclerView.LayoutParams;

import java.util.List;

import static com.weishop.test.list.recycler.RecyclerView.NO_POSITION;

/**
 * A {@link RecyclerView .LayoutManager} implementation which provides similar
 * functionality to {@link android.widget.ListView}.
 */
public class LinearLayoutManager extends RecyclerView.LayoutManager implements
    ...
    /**
     * Constructor used when layout manager is set in XML by RecyclerView
     * attribute "layoutManager". Defaults to vertical orientation.
     *
     * @attr ref android.support.v7.recyclerview.R.styleable#
     *       RecyclerView_android_orientation
     * @attr ref
     *       android.support.v7.recyclerview.R.styleable#RecyclerView_reverseLayout
     * @attr ref
     *       android.support.v7.recyclerview.R.styleable#RecyclerView_stackFromEnd
     */
     ...
     * If {@link RecyclerView } is LTR, than it will layout from RTL, if
     * {@link RecyclerView } is RTL, it will layout from LTR. If you are looking
     * for the exact same behavior of
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        // If the previous scroll delta was less than zero, the extra space
        // should be laid out
    ...
    private void layoutForPredictiveAnimations(RecyclerView.Recycler recycler,
        ...
        for (int i = 0; i < scrapSize; i++) {
            ...
            final int direction = position < firstChildPos != mShouldReverseLayout ? LayoutState.LAYOUT_START
    ...
    private void updateAnchorInfoForLayout(RecyclerView.Recycler recycler,
    ...
    private boolean updateAnchorFromChildren(RecyclerView.Recycler recycler,
        ...
        View referenceChild = anchorInfo.mLayoutFromEnd ? findReferenceChildClosestToEnd(recycler,
                state) : findReferenceChildClosestToStart(recycler, state);
    ...
    private boolean updateAnchorFromPendingData(RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        if (mPendingScrollPositionOffset == INVALID_OFFSET) {
            ...
            if (child != null) {
                ...
                anchorInfo.mCoordinate = anchorInfo.mLayoutFromEnd ? (mOrientationHelper
                        .getDecoratedEnd(child) + mOrientationHelper.getTotalSpaceChange())
     ...
     * {@link #getReverseLayout()}, {@link ViewCompat#getLayoutDirection(View)}
    ...
    private int computeScrollOffset(RecyclerView.State state) {
        ...
        return ScrollbarHelper.computeScrollOffset(state, mOrientationHelper,
                ...
                findFirstVisibleChildClosestToEnd(!mSmoothScrollbarEnabled, true), this,
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        layoutDecorated(view, left + params.leftMargin, top + params.topMargin, right
                - params.rightMargin, bottom - params.bottomMargin);
        if (DEBUG) {
    ...
    /**
     * Convenience method to find the visible child closes to start. Caller
     * should check if it has enough children.
     *
     * @param completelyVisible Whether child should be completely visible or
     *            not
     * @return The first visible child closest to start of the layout from
     *         user's perspective.
     */
     ...
     * check if it has enough children.
     *
     * @param completelyVisible Whether child should be completely visible or
     *            not
    ...
    View findReferenceChild(RecyclerView.Recycler recycler, RecyclerView.State state, int start,
            int end, int itemCount) {
        ensureLayoutState();
    ...
    /**
     * Returns the adapter position of the first fully visible view. This
     * position does not include adapter changes that were dispatched after the
     * last layout pass.
     * <p>
    ...
    /**
     * Returns the adapter position of the last visible view. This position does
     * not include adapter changes that were dispatched after the last layout
     * pass.
     * <p>
     * Note that, this value is not affected by layout orientation or item order
     * traversal. ({@link #setReverseLayout(boolean)}). Views are sorted by
    ...
    /**
     * Returns the adapter position of the last fully visible view. This
     * position does not include adapter changes that were dispatched after the
     * last layout pass.
     * <p>
     * Note that bounds check is only performed in the current orientation. That
        ...
        if (mShouldReverseLayout) {
            if (dropDirection == LayoutState.ITEM_DIRECTION_TAIL) {
                scrollToPositionWithOffset(
                        targetPos,
                        mOrientationHelper.getEndAfterPadding()
                                - (mOrientationHelper.getDecoratedStart(target) + mOrientationHelper
                                        .getDecoratedMeasurement(view)));
            } else {
                scrollToPositionWithOffset(targetPos, mOrientationHelper.getEndAfterPadding()
            ...
            if (dropDirection == LayoutState.ITEM_DIRECTION_HEAD) {
                ...
                scrollToPositionWithOffset(targetPos, mOrientationHelper.getDecoratedEnd(target)
    ...
    class AnchorInfo {
        ...
        public String toString() {
            return "AnchorInfo{" + "mPosition=" + mPosition + ", mCoordinate=" + mCoordinate
        ...
        public void assignFromViewAndKeepVisibleRect(View child) {
            ...
            if (mLayoutFromEnd) {
                ...
                // ensure we did not push child's top out of bounds because of
                // this
                if (previousEndMargin > 0) {// we have room to shift bottom if
                                            // necessary
                    final int childSize = mOrientationHelper.getDecoratedMeasurement(child);
