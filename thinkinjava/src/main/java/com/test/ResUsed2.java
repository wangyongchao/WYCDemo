...
public class ResUsed2 {

    public static void main(String[] args) {
        ResUsed2 res = new ResUsed2();
        res.soSize();

    }

    public void soSize() {
        String dir = "D:\\tongji\\app-xesmarket-Release (1)\\lib\\armeabi-v7a";
    ...
    public void tongjimode() {
        ResUsed2 res = new ResUsed2();
        String[] modes = {"thumbs", "flower", "language", "pk", "agora", "roleplay", "contiright"
                , "hotword", "hongbao"};
        for (int i = 0; i < modes.length; i++) {
            String dir = "C:\\Users\\wangy\\Downloads\\livebusiness\\drawable-xxhdpi";
            res.resSize(modes[i], dir);
        }
    }

    public void resSize(String mode, String dir) {
        File checkDir = new File(dir);
        File[] files = checkDir.listFiles();
        long sumSize = 0;
        for (int i = 0; i < files.length; i++) {
            ...
            if (name.contains(mode)) {
                sumSize = sumSize + file.length();
            }
        }

        System.out.println("mode=" + mode + ",size=" + FormetFileSize(sumSize));

    }

    public void checkRes() {
//		File javaDir = new File(
//				"/Users/linyuqiang/AndroidStudioProjects/feature_record_live/xueersiwangxiao
//				/business/livevideo/src/main");
        String dir = "E:\\workspace\\xueersiwangxiao_other\\xueersiwangxiao\\business-base" +
                "\\livevideo";
        File javaDir = new File(dir);
//		File xmlDir = new File(
//				"/Users/linyuqiang/AndroidStudioProjects/feature_record_live/xueersiwangxiao
//				/business/livevideo/src/main/res");
                    ...
                    for (int j = 0; j < size; j++) {
                        ...
                        for (int r = 0; r < res.size(); r++) {
                            ...
                            if (re.getName().startsWith(drawableling)) {
                                ...
                                r--;
                                System.out.println(
                                        "drawable_xml\t" + remove + "\t" + drawableling + ",size" +
                                                "=" + drawEntity.size);
//								break;
                            }
                        }
...
//						System.out.println("drawable\t" + drawableling);
                        ...
                        if (index != -1) {
                            ...
                            for (int r = 0; r < res.size(); r++) {
                                DrawEntity drawEntity = res.get(r);
                                File re = drawEntity.file;
//								if (re.getPath().contains("bg_live_ke_energy_quanli_icon_normal
//								.png")) {
...
//							System.out.println("drawable1\t" + remove + ",name=" + javaFile
//							.getName() + "\t" + drawable
//									+ ",javaFile=" + javaFile);
                            if (remove) {
                            ...
                            for (int r = 0; r < xmlSizes.size(); r++) {
                                ...
                                if (re.filename.startsWith(layout)) {
                                    ...
                                    if (remove) {
                                        ...
                                        System.out.println("mode=" + mode + "\t" + layout + "," +
                                                "size=" + re.size);
                                    }
                                    r--;
//									break;
