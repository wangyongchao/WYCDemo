package com.weishop.test.list.draglistview.lib;
...
public class DragSortController extends SimpleFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener {
     ...
     * @return True if drag started, false otherwise. 开始拖拽的点
     */
    ...
    public boolean onTouch(View v, MotionEvent ev) {
        ...
        if (TestUtils.OPEN) {
            System.out.println("DragSortController onTouch action=" + strAction + ",x=" + x1
                    + ",y=" + y1 + ",rawX=" + rawX + ",rawY=" + rawY);
        }
    ...
    public int viewIdHitPosition(MotionEvent ev, int id) {
        ...
        int touchPos = mDslv.pointToPosition(x, y); // 根据坐标获取点击item对应的位置 includes headers/footers

        final int numHeaders = mDslv.getHeaderViewsCount();
        final int numFooters = mDslv.getFooterViewsCount();
        final int count = mDslv.getCount();//包括header和footer

        // Log.d("mobeta", "touch down on position " + itemnum);
        // We're only interested if the touch was on an
        // item that's not a header or footer. 只在item中，不包括header和footer
        if (touchPos != AdapterView.INVALID_POSITION && touchPos >= numHeaders
                && touchPos < (count - numFooters)) {
            final View item = mDslv.getChildAt(touchPos - mDslv.getFirstVisiblePosition());//获取点中的view
            final int rawX = (int) ev.getRawX();
            final int rawY = (int) ev.getRawY();

            View dragBox = id == 0 ? item : (View) item.findViewById(id);
            if (dragBox != null) {
                dragBox.getLocationOnScreen(mTempLoc);//拖拽view，在屏幕中的位置，从屏幕左上角算起,包括状态栏，标题栏.

                if (rawX > mTempLoc[0] && rawY > mTempLoc[1] &&//
                        rawX < mTempLoc[0] + dragBox.getWidth() &&
                        rawY < mTempLoc[1] + dragBox.getHeight()) {//判断点击的点是否在拖拽view内.
    ...
    public boolean onDown(MotionEvent ev) {
        TestUtils.printEvent(ev, "DragSortController onDown", null);
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        TestUtils.printEvent(e2, "DragSortController onScroll", map);
    ...
    public void onLongPress(MotionEvent e) {
        TestUtils.printEvent(e, "DragSortController onLongPress", null);
