...
public class DragSortListView extends ListView {
     ...
     * of the ListView height, thus screen resolution agnostic.与屏幕分辨率无关
     */
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (TestUtils.OPEN) {
            System.out.println("DragSortListView onTouchEvent action=" + strAction + ",x=" + x
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (TestUtils.OPEN) {
            System.out.println("DragSortListView onInterceptTouchEvent action=" + strAction + ",x=" + x
                    + ",y=" + y + ",rawX=" + rawX + ",rawY=" + rawY + ",getLeft=" + getLeft()
                    + ",getTop=" + getTop() + ",getRight=" + getRight() + ",getbottom="
                    + getBottom());
        }
    ...
    /**
     * AbsListView在onlayout的时候调用次方，对子view进行布局.
     */
    @Override
    ...
    private void updateFloatView() {
        ...
        if (firstPos < numHeaders) {//如果当前list的header还在屏幕内，则获取最后一个header的view
        ...
        if (lastPos >= getCount() - numFooters - 1) {//如果当前list的footer在屏幕内，则获取第一个footer的view
